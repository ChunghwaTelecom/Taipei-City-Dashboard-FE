...
def parse_postpartum_care_center(file_path: str) -> pd.DataFrame:
    ...
    locality_ids = [
        map_box_geo_data["locality_id"] for map_box_geo_data in map_box_geo_data_list
    ]
    locality_texts = [
        map_box_geo_data["locality_text"] for map_box_geo_data in map_box_geo_data_list
    ]
...
def output_postpartum_care_center(processed_df: pd.DataFrame):
    ...
    with open(
        "data/clean_data/processed_postpartum_care_center.json", "w", encoding="utf-8"
    ) as file:
...
def main():
    ...
    processed_df: pd.DataFrame = parse_postpartum_care_center(
        postpartum_care_center_file_path
    )
    output_postpartum_care_center(processed_df)
